...
h264_nal_buffer_cb(SIPE_UNUSED_PARAMETER GstPad *pad,
		   ...
		   SIPE_UNUSED_PARAMETER gpointer user_data)
{
	...
	while (data < map.data + map.size) {
		...
		SIPE_DEBUG_INFO("found NAL of size %u", size);
		++nal_count;
	}

	SIPE_DEBUG_INFO("it has %u NALs", nal_count);
...
find_payloader(GValue *value, GstCaps *rtpcaps)
{
	...
	GST_WARNING("***** %s caps are %" GST_PTR_FORMAT, GST_ELEMENT_NAME(element), caps);
